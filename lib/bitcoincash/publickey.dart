...
//        _publicKey = ECPublicKey((_point), _domainParams);
  ...
  BCHPublicKey.fromXY(BigInt x, BigInt y, {bool compressed = true}) {
...
//        _publicKey = ECPublicKey(_point, _domainParams);
  ...
  BCHPublicKey.fromDER(List<int> buffer, {bool strict = true}) {
    if (buffer.isEmpty) {
...
//        _publicKey = ECPublicKey(_point, _domainParams);
  ...
  ///
  /// [strict] - If *true* then we enforce strict DER encoding rules. Defaults to *true*.
  BCHPublicKey.fromHex(String pubkey, {bool strict = true}) {
    if (pubkey.trim() == '') {
