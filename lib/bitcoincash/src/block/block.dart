...
class Block {
  ...
  static final NULL_HASH = HEX.decode(
  ...
  List<List<int>> getMerkleTree() {
    ...
    var j = 0;
    for (var size = transactions.length;
        size > 1;
        size = ((size + 1) / 2).floor()) {
  ...
  void _fromBuffer(List<int> blockbuf, {bool rawDataRead = false}) {
    ...
    var dataReader = ByteDataReader()..add(blockbuf);

    if (rawDataRead) {
      dataReader.read(
          START_OF_BLOCK); // consume first eight bytes if reading raw data
    }
  ...
  List<int> get buffer {
    ...
    var txBuf = _transactions.fold(
        <int>[],
        (List<int> prev, Transaction tx) =>
            prev + HEX.decode(tx.serialize(performChecks: false)));
