...
class BlockHeader {
  ...
  static final LARGEST_HASH = BigInt.parse(
  ...
  BlockHeader(this._version, this._prevHash, this._merkleRoot, this._time,
      this._bits, this._nonce);
  ...
  BlockHeader.fromJSONMap(LinkedHashMap<String, dynamic> map) {
    ...
    _prevHash = HEX.decode(map['prevHash']).reversed.toList();
    _merkleRoot = HEX.decode(map['merkleRoot']).reversed.toList();
    _time = map['time'];
    _bits = map['bits'];
    _nonce = map['nonce'];
  }
  ...
  void _parseBuffer(List<int> buffer, {bool rawDataRead = false}) {
    ...
    var byteDataReader = ByteDataReader()..add(buffer);
