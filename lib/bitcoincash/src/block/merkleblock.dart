...
import 'package:cashew/bitcoincash/src/exceptions.dart';
...
class MerkleBlock {
  ...
  bool validMerkleTree() {
    ...
    var resultMap = _traverseMerkleTree(height, 0, flagBitsUsed: 0);
  ...
  List<String> filteredTxsHash() {
    ...
    if (hashes.length > numTransactions) {
      throw MerkleTreeException(
          'Invalid merkle tree - more hashes than transactions');
    }

    // Can't have more flag bits than num hashes
    if (flags.length * 8 < hashes.length) {
      throw MerkleTreeException(
          'Invalid merkle tree - more flag bits than hashes');
    }
    ...
    var result = _traverseMerkleTree(height, 0,
        hashesUsed: hashesUsed, flagBitsUsed: flagBitsUsed, checkForTxs: true);
  ...
  Map<String, dynamic> _traverseMerkleTree(int depth, int pos,
      {List<String> txs,
      int hashesUsed = 0,
      int flagBitsUsed = 0,
      bool checkForTxs = false}) {
    // TODO: FIX I hate this, but optional params must be const. A lot of 'out' params in this method. Refactor!
    txs = txs ?? [];
    ...
    var isParentOfMatch = (flags[flagBitsUsed >> 3] >> (flagBitsUsed++ & 7)) &
        1; // fucking magic math *facepalm*
    if (depth == 0 || isParentOfMatch == 0) {
      ...
      return {
        'nodeValue': HEX.decode(hash),
        'hashesUsed': hashesUsed,
        'flagBitsUsed': flagBitsUsed
      };
    } else {
      var result = _traverseMerkleTree(depth - 1, pos * 2,
          txs: txs,
          hashesUsed: hashesUsed,
          flagBitsUsed: flagBitsUsed,
          checkForTxs: checkForTxs);
      ...
      if (pos * 2 + 1 < _calcTreeWidth(depth - 1)) {
        result = _traverseMerkleTree(depth - 1, pos * 2 + 1,
            txs: txs,
            hashesUsed: hashesUsed,
            flagBitsUsed: flagBitsUsed,
            checkForTxs: checkForTxs);
      ...
      if (checkForTxs) {
        ...
        return {
          'nodeValue': sha256Twice((left as List<int>) + (right as List<int>)),
          'hashesUsed': hashesUsed,
          'flagBitsUsed': flagBitsUsed
        };
      }
  ...
  void _decodeObject(Map<String, dynamic> blockObject) {
    ...
    _header = BlockHeader(
        headerObj['version'],
        HEX.decode(headerObj['prevHash']).reversed.toList(),
