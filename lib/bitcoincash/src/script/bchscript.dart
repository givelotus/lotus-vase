...
///
class BCHScript with ScriptBuilder {
  ...
  /// E.g.
  /// ```
  /// var script = BCHScript.fromString('OP_0 OP_PUSHDATA4 3 0x010203 OP_0');
  /// ```
  BCHScript.fromString(String script) {
  ...
  /// E.g.
  /// ```
  /// var script = BCHScript.fromHex('76a914f4c03610e60ad15100929cc23da2f3a799af172588ac');
  /// ```
  ///
  ...
  BCHScript.fromChunks(List<ScriptChunk> chunks) {
    _chunks = chunks;
    _convertChunksToByteArray();
  }
  ...
  BCHScript.fromByteArray(Uint8List buffer) {
  ...
  BCHScript.fromBuffer(Uint8List buffer) {
    _processBuffer(buffer);
  }
  ...
  BCHScript.fromBitcoindString(String str) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      ...
      if (token.startsWith('0x')) {
        ...
        tbuf = BCHScript().add(utf8.encode(tstr)).buffer;
        bw.write(tbuf);
...
//          var buf = Buffer.from(tokens[i], 'hex')
  ...
  void _convertChunksToByteArray() {
...
//        _byteArray = Uint8List.fromList(HEX.decode(chunkString.replaceAll(' ', '')));
  ...
  void _processBuffer(Uint8List buffer) {
    ...
    while (byteDataReader.remainingLength > 0) {
      try {
        ...
        if (opcodenum > 0 && opcodenum < OpCodes.OP_PUSHDATA1) {
          len = opcodenum;
          buf = byteDataReader.remainingLength >= len
  ...
  void _processChunks(String script) {
    ...
    for (var index = 0; index < tokenList.length;) {
      ...
      if (opcodenum == null) {
        ...
        if (opcodenum > 0 && opcodenum < OpCodes.OP_PUSHDATA1) {
          _chunks.add(ScriptChunk(HEX.decode(tokenList[index + 1].substring(2)),
              opcodenum, opcodenum));
  ...
  String toString({type = 'hex'}) {
    if (_chunks.isNotEmpty) {
      return _chunks
          .fold(
              '',
              (String prev, ScriptChunk chunk) =>
                  prev + _chunkToString(chunk, type: type))
          .trim();
    }
  ...
  BCHScript removeCodeseparators() {
  ...
  BCHScript findAndDelete(BCHScript tmpScript) {
    var buf = List<int>.from(tmpScript.buffer);
    var hex = HEX.encode(buf);
    for (var i = 0; i < _chunks.length; i++) {
      var script2 = BCHScript.fromChunks([_chunks[i]]);
