...
import 'package:cashew/bitcoincash/src/encoding/utils.dart';
...
class Interpreter {
  ...
  Interpreter.fromScript(BCHScript script, int flags) {
    _script = script;
    _flags = flags;
  }
  ...
  bool verifyScript(BCHScript scriptSig, BCHScript scriptPubkey,
      {Transaction tx, int nin = 0, int flags = 0, BigInt satoshis}) {
    tx ??= Transaction();
    ...
    if (flags & ScriptFlags.SCRIPT_ENABLE_SIGHASH_FORKID != 0) {
      ...
      if (satoshis == null) {
        throw ScriptException(
            'internal error - need satoshis to verify FORKID transactions');
      }
    }
    ...
    if ((flags & ScriptFlags.SCRIPT_VERIFY_SIGPUSHONLY) != 0 &&
    ...
    if ((flags & ScriptFlags.SCRIPT_VERIFY_P2SH != 0) &&
      ...
      var redeemScript =
          BCHScript.fromByteArray(Uint8List.fromList(redeemScriptSerialized));
      stackCopy.pop();
  ...
  bool checkSignatureEncoding(List<int> buf, int flags) {
    var sig;
    // TODO: FIX Error string never used
    // var errStr;
  ...
  bool evaluate() {
    ...
    if (_script.buffer.length > 10000) {
    ...
    try {
      while (_pc < _script.chunks.length) {
        var thisStep = {'pc': _pc, 'opcode': _script.chunks[pc].opcodenum};
  ...
  bool _step() {
    ...
    var fRequireMinimal = (flags & ScriptFlags.SCRIPT_VERIFY_MINIMALDATA) !=
        0; // TODO: FIX This is somehow used in JS BigNumber class
    ...
    _pc++; // TODO: FIX global var f*ckery. Looks like index pointer into script chunks
    ...
    if (fExec && opcodenum >= 0 && opcodenum <= OpCodes.OP_PUSHDATA4) {
      ...
      if (chunk.len != chunk.buf.length) {
        ...
        (OpCodes.OP_IF <= opcodenum && opcodenum <= OpCodes.OP_ENDIF)) {
      switch (opcodenum) {
        // Push value
...
//                case OpCodes.OP_NOP2: // same numeric as CHECKLOCKTIMEVERIFY. Core buggery.
        case OpCodes.OP_CHECKLOCKTIMEVERIFY:
          if (!(flags & ScriptFlags.SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY != 0)) {
            ...
            if (flags & ScriptFlags.SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_NOPS !=
                0) {
          ...
          var nLockTime = fromScriptNumBuffer(
              Uint8List.fromList(_stack.peek()), fRequireMinimal,
              nMaxNumSize: 5);
...
//      case OpCodes.OP_NOP3:
        case OpCodes.OP_CHECKSEQUENCEVERIFY:
          ...
          var nSequence = fromScriptNumBuffer(
              Uint8List.fromList(_stack.peek()), fRequireMinimal,
              nMaxNumSize: 5);
          ...
          if ((nSequence &
                  BigInt.from(ScriptFlags.SEQUENCE_LOCKTIME_DISABLE_FLAG)) !=
              BigInt.zero) {
            break;
          }
        ...
        case OpCodes.OP_2ROT:
          ...
          spliced = _stack.splice(
              _stack.length - 6, 2); // TODO: FIX Splice needs IMPLEMENTATION
...
//                    buf = HEX.decode(BigInt.from(_stack.length).toRadixString(16));
          if (_stack.length == 0) {
            buf =
                []; // don't push array with zero value, push empty string instead
          }
        ...
        case OpCodes.OP_SIZE:
          ...
          bn = BigInt.from(stack.peek().length);
...
//                    _stack.push(HEX.decode(bn.toRadixString(16)));
        ...
        // TODO: FIX Using a List<int> for the stack seems to be problematic under certain circumstances
        ...
        case OpCodes.OP_INVERT:
          ...
          stack.replaceAt(
              stack.length - 1, buf); // replace item at top with modified value
          break;
...
//                        bn2 = BigInt.tryParse(HEX.encode(stack.peek()), radix: 16) ?? BigInt.zero;
            ...
            bn2 = fromScriptNumBuffer(
                Uint8List.fromList(stack.peek()), fRequireMinimal);
            ...
            var padding =
                shifted.toRadixString(16).padLeft(buf1.length * 2, '0');

            if (n > 0) {
              var shiftedList = HEX.decode(padding);
              _stack
                  .push(shiftedList.sublist(shiftedList.length - buf1.length));
            } else {
              _stack.push(HEX.decode(shifted.toRadixString(
                  16))); // if no shift occured then don't drop bits
            }
          }
          break;
        ...
        case OpCodes.OP_EQUALVERIFY:
          ...
          _stack.push(fEqual
              ? TRUE
              : FALSE); // TODO: FIX pushing true and false to stack. Is works ?
...
//                    _stack.push(HEX.decode(bn.toRadixString(16)));
        ...
        case OpCodes.OP_MAX:
          ...
          bn1 = fromScriptNumBuffer(
              Uint8List.fromList(stack.peek(index: -2)), fRequireMinimal);
          bn2 = fromScriptNumBuffer(
              Uint8List.fromList(stack.peek()), fRequireMinimal);
          ...
          switch (opcodenum) {
            ...
            case OpCodes.OP_MOD:
              ...
              // TODO: FIX Is this re-enabled OP_CODE supposed to work in this fucked-up way !?
              bn = bn1.abs() %
                  bn2.abs(); // seriously ? I have to convert to abs() to get correct result if bn1 < 0. WTF Bitcoin ?

              if (bn1.isNegative) {
                bn =
                    -bn; // flip sign to conform to weird bitcoin mod behaviour. WTF!?
              }
              break;

            case OpCodes.OP_BOOLAND:
              if ((bn1.compareTo(BigInt.zero) != 0) &&
                  (bn2.compareTo(BigInt.zero) != 0)) {
                bn = BigInt.one;
              } else {
                bn = BigInt.zero;
              }
...
//                            bn = (bn1 == BigInt.zero && bn2 == BigInt.zero) ? BigInt.zero : BigInt.one;
            ...
            case OpCodes.OP_BOOLOR:
              bn = (bn1 != BigInt.zero || bn2 != BigInt.zero)
                  ? BigInt.one
                  : BigInt.zero;
              break;
        ...
        case OpCodes.OP_WITHIN:
          ...
          bn1 = fromScriptNumBuffer(
              Uint8List.fromList(stack.peek(index: -3)), fRequireMinimal);
          bn2 = fromScriptNumBuffer(
              Uint8List.fromList(stack.peek(index: -2)), fRequireMinimal);
          var bn3 = fromScriptNumBuffer(
              Uint8List.fromList(stack.peek()), fRequireMinimal);
        ...
        case OpCodes.OP_CHECKSIGVERIFY:
          ...
          if (!checkSignatureEncoding(bufSig, flags) ||
              !checkPubkeyEncoding(bufPubkey)) {
            return false;
          }

          // Subset of script starting at the most recent codeseparator
          var subscript =
              BCHScript.fromChunks(_script.chunks.sublist(pbegincodehash));

          // Drop the signature, since there's no way for a signature to sign itself
          var tmpScript = BCHScript().add(bufSig);
          subscript.findAndDelete(tmpScript);

          try {
            pubkey = BCHPublicKey.fromHex(HEX.encode(bufPubkey), strict: false);
            sig = BCHSignature.fromTxFormat(HEX.encode(
                bufSig)); // TODO: FIX Why can't I construct a BCHSignature that properly verifies from TxFormat ???

            fSuccess = _tx.verifySignature(
                sig, pubkey, _nin, subscript, _satoshis, _flags);
          } catch (e) {
          ...
          if (!fSuccess &&
              (flags & ScriptFlags.SCRIPT_VERIFY_NULLFAIL != 0) &&
              bufSig.isNotEmpty) {
        ...
        case OpCodes.OP_CHECKMULTISIGVERIFY:
          ...
          var nKeysCount = fromScriptNumBuffer(
                  Uint8List.fromList(stack.peek(index: -i)), fRequireMinimal)
              .toInt();
//                    var nKeysCount = BigInt.parse(HEX.encode(stack.peek(index: -i)), radix: 16).toInt();
          ...
          var nSigsCount = fromScriptNumBuffer(
                  Uint8List.fromList(stack.peek(index: -i)), fRequireMinimal)
              .toInt();
//                    var nSigsCount = BigInt.parse(HEX.encode(stack.peek(index: -i)), radix: 16).toInt();
          ...
          for (var k = 0; k < nSigsCount; k++) {
            ...
            subscript.findAndDelete(BCHScript().add(bufSig));
          }
          ...
          while (fSuccess && nSigsCount > 0) {
            ...
            try {
              pubkey =
                  BCHPublicKey.fromHex(HEX.encode(bufPubkey), strict: false);
              sig = BCHSignature.fromTxFormat(HEX.encode(bufSig));

              fOk = _tx.verifySignature(
                  sig, pubkey, _nin, subscript, _satoshis, _flags);
            } catch (e) {
              // invalid sig or pubkey
              fOk = false;
          ...
          if ((flags & ScriptFlags.SCRIPT_VERIFY_NULLDUMMY != 0) &&
              stack.peek().isNotEmpty) {
        ...
        case OpCodes.OP_SPLIT:
          ...
          var position = fromScriptNumBuffer(
                  Uint8List.fromList(stack.peek()), fRequireMinimal)
              .toInt();
...
//                    var size = BigInt.parse(HEX.encode(stack.peek()), radix: 16).toInt();
  ...
  bool checkLockTime(BigInt nLockTime) {
    ...
    if (!((_tx.nLockTime < Interpreter.LOCKTIME_THRESHOLD &&
            nLockTime < (Interpreter.LOCKTIME_THRESHOLD_BN)) ||
        (_tx.nLockTime >= Interpreter.LOCKTIME_THRESHOLD &&
            nLockTime >= (Interpreter.LOCKTIME_THRESHOLD_BN)))) {
      return false;
    }
  ...
  bool checkSequence(BigInt nSequence) {
    ...
    var nLockTimeMask = ScriptFlags.SEQUENCE_LOCKTIME_TYPE_FLAG |
        ScriptFlags.SEQUENCE_LOCKTIME_MASK;
    ...
    // transaction.
    var SEQUENCE_LOCKTIME_TYPE_FLAG_BN =
        BigInt.from(ScriptFlags.SEQUENCE_LOCKTIME_TYPE_FLAG);

    if (!((txToSequenceMasked < SEQUENCE_LOCKTIME_TYPE_FLAG_BN &&
  ...
  bool checkPubkeyEncoding(List<int> pubkey) {
    if ((flags & ScriptFlags.SCRIPT_VERIFY_STRICTENC) != 0 &&
        !BCHPublicKey.isValid(HEX.encode(pubkey))) {
