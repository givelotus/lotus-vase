...
import 'package:cashew/bitcoincash/src/script/scriptflags.dart';
import 'package:cashew/bitcoincash/src/transaction/transaction.dart';
import 'package:cashew/bitcoincash/src/transaction/transaction_input.dart';

// import 'package:cashew/bitcoincash/src/script/P2PKHScriptSig.dart';
...
class Sighash {
  ...
  ///
  /// [subscript] - The portion of [BCHScript] in the [TransactionOutput] of Spent [Transaction] (after OP_CODESEPERATOR) that will be covered by the signature
  ///
  /// [flags] - The bitwise combination of [ScriptFlags] related to Sighash. Applies to BCH only,
  ...
  String hash(Transaction txn, int sighashType, int inputNumber,
    ...
    _txn = txnCopy;
    var subscriptCopy = BCHScript.fromHex(subscript.toHex()); // make a copy
    ...
    if ((sighashType & SighashType.SIGHASH_FORKID != 0) &&
        (flags & ScriptFlags.SCRIPT_ENABLE_SIGHASH_FORKID != 0)) {
      return HEX.encode(_sigHashForForkid(
...
// TODO: FIX Unused variable commented?
      ...
      txnCopy.outputs[inputNumber] =
          txout; // FIXME : ??? Is this the correct way ?

    }

    if (_sighashType & SighashType.SIGHASH_ANYONECANPAY > 0) {
      var keepTxn = _txn.inputs[inputNumber];
...
// TODO: FIX Is this needed?
  ...
  List<int> getHash() {
    ...
    writer.writeInt32(_sighashType, Endian.little);
...
//    }
  ...
  List<int> _sigHashForForkid(Transaction txn, int sighashType, int inputNumber,
    ...
    if ((sighashType & 31) != SighashType.SIGHASH_SINGLE &&
        (sighashType & 31) != SighashType.SIGHASH_NONE) {
      hashOutputs = GetOutputsHash(txn);
    } else if ((sighashType & 31) == SighashType.SIGHASH_SINGLE &&
        inputNumber < txn.outputs.length) {
