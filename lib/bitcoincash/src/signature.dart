...
import 'package:cashew/bitcoincash/src/privatekey.dart';
import 'package:cashew/bitcoincash/src/publickey.dart';
...
class BCHSignature {
  ...
  BCHSignature.fromTxFormat(String buffer) {
    // TODO: FIX Add guards to assert TxFormat
    var decoded = HEX.decode(buffer);
    var nhashtype = decoded[decoded.length - 1];

    var derbuf = decoded.sublist(0, decoded.length - 1);
    _nhashtype =
        nhashtype; // this is OK. SighashType is already represented as HEX. No decoding needed
  ...
  BCHSignature.fromPrivateKey(BCHPrivateKey privateKey) {
    var privKey = ECPrivateKey(privateKey.privateKey, _domainParams);
  ...
  BCHSignature.fromPublicKey(BCHPublicKey publicKey) {
    var pubKey = ECPublicKey(publicKey.point, _domainParams);
    _publicKey = publicKey;
  ...
  ///
  BCHSignature.fromCompact(List<int> buffer, List<int> signedMessage) {
    var compressed = true;
...
// can be forced to do hex encoding via HEX.encode(utf8.encode())
  ...
  BCHPublicKey _recoverPublicKey(int i, List<int> hashBuffer) {
    if (![0, 1, 2, 3].contains(i)) {
      throw SignatureException('i must be equal to 0, 1, 2, or 3');
    }
