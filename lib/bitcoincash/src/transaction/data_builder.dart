...
import 'package:cashew/bitcoincash/src/script/opcodes.dart';
import 'package:cashew/bitcoincash/src/script/bchscript.dart';
import 'package:cashew/bitcoincash/src/transaction/locking_script_builder.dart';
import 'dart:math';
...
mixin DataLockMixin on _DataLockBuilder implements LockingScriptBuilder {
  ...
  BCHScript getScriptPubkey() {
    if (dataStack == null || dataStack.isEmpty) {
      return BCHScript.fromString('OP_FALSE OP_RETURN');
    }
      ...
      if (entry != null && HEX.encode(entry).isNotEmpty) {
        ...
        if (len < OpCodes.OP_PUSHDATA1) {
          ...
          scriptPubkey = scriptPubkey +
              sprintf(' %s %s 0x%s', [opcodenum, len, encodedData]);
        }
      }
    });

    return BCHScript.fromString(scriptPubkey);
  }
}
...
abstract class _DataLockBuilder implements LockingScriptBuilder {
  ...
  void fromScript(BCHScript script) {
    if (script != null && script.buffer != null && script.chunks.length == 3) {
