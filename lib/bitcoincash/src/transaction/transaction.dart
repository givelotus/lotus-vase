...
import 'package:cashew/bitcoincash/src/signature.dart';
import 'package:cashew/bitcoincash/src/transaction/data_builder.dart';
import 'package:cashew/bitcoincash/src/transaction/default_builder.dart';
import 'package:cashew/bitcoincash/src/transaction/signed_unlock_builder.dart';
import 'package:cashew/bitcoincash/src/transaction/transaction_input.dart';
import 'package:cashew/bitcoincash/src/transaction/transaction_output.dart';
...
class Transaction {
  ...
  Transaction.fromJSONMap(LinkedHashMap<String, dynamic> map) {
    ...
    (map['inputs'] as List).forEach((input) {
      var tmpTx = TransactionInput(input['prevTxId'], input['outputIndex'],
          BCHScript(), BigInt.zero, input['sequenceNumber']);
      tmpTx.scriptBuilder.fromScript(BCHScript.fromHex(input['script']));
    ...
    (map['outputs'] as List).forEach((output) {
      ...
      txOut.script = BCHScript.fromHex(output['script']);
  ...
  Transaction.fromHex(String txnHex) {
    var hash = sha256Twice(HEX.decode(txnHex)).reversed.toList();
    _txHash = hash;
  ...
  List<int> _getHash() {
    var hash = sha256Twice(HEX.decode(serialize(performChecks: false)));
    return hash;
  }
  ...
  Transaction spendTo(Address recipient, BigInt sats,
    ...
    if (sats <= BigInt.zero) {
      throw TransactionAmountException(
          'You can only spend a positive amount of satoshis');
    }
...
//        txnOutput.script = scriptBuilder.getScriptPubkey();
  ...
  Transaction addData(List<int> data, {DataLockBuilder scriptBuilder}) {
  ...
  Transaction spendFromOutput(TransactionOutput utxo, int sequenceNumber,
    ...
    var input = TransactionInput(utxo.transactionId, utxo.outputIndex,
        utxo.script, utxo.satoshis, sequenceNumber,
  ...
  Transaction spendFromMap(Map<String, Object> map,
      {UnlockingScriptBuilder scriptBuilder}) {
    // TODO: FIX More robust validation / error handling needed here.
  ...
  void signInput(int index, BCHPrivateKey privateKey, {sighashType = 0}) {
    if (index + 1 > _txnInputs.length) {
      throw TransactionException(
          'Input index out of range. Max index is ${_txnInputs.length + 1}');
    } else if (_txnInputs.isEmpty) {
      throw TransactionException(
          'No Inputs defined. Please add some Transaction Inputs');
    }

    _sign(_txnInputs[index], privateKey, sighashType: sighashType);
  }

  void _sign(TransactionInput input, BCHPrivateKey privateKey,
      {sighashType = SighashType.SIGHASH_ALL | SighashType.SIGHASH_FORKID}) {
    // TODO: FIX This is a test work-around for why I can't sign an unsigned raw txn
    // TODO: FIX This assumes we're signing P2PKH
    ...
    var sigHash = Sighash();
    var hash =
        sigHash.hash(this, sighashType, inputIndex, subscript, input.satoshis);

    // TODO: FIX Revisit this issue surrounding the need to sign a reversed copy of the hash.
    ...
    var sig = BCHSignature.fromPrivateKey(privateKey);
  ...
  // TODO: FIX Check under which circumstances this long list of params is actually required. Can be trimmed ?
  bool verifySignature(BCHSignature sig, BCHPublicKey pubKey, int inputNumber,
      BCHScript subscript, BigInt satoshis, int flags) {
    var sigHash = Sighash();
    var hash = sigHash.hash(
        this, sig.nhashtype, inputNumber, subscript, satoshis,
        flags: flags);
  ...
  Transaction lockUntilBlockHeight(int blockHeight) {
    ...
    // TODO: FIX assumption on the length of _nLockTime. Risks indexexception
  ...
  dynamic get lockTime {
    // TODO: FIX Figure out how to use Type System to force consumer of this
  ...
  String verify() {
    ...
    // Size limits
    if (serialize(performChecks: false).length > MAX_BLOCK_SIZE) {
...
//
  ...
  void _checkForMissingSignatures() {
    if (transactionOptions.contains(TransactionOption.DISABLE_FULLY_SIGNED)) {
      return;
    }
  ...
  BigInt _getUnspentValue() {
    var inputAmount = _inputTotals();
    var outputAmount = _txnOutputs.fold(BigInt.zero,
        (BigInt prev, TransactionOutput elem) => prev + elem.satoshis);
  ...
  BigInt _estimateFee() {
    ...
    var available = _getUnspentValue();
  ...
  int _estimateSize() {
    ...
    _txnOutputs.forEach((output) {
      result += HEX.decode(output.script.toHex()).length +
          9; // <---- HOW DO WE CALCULATE SCRIPT FROM JUST AN ADDRESS !? AND LENGTH ???
    });
  ...
  void _sortInputs(List<TransactionInput> txns) {
    txns.sort((lhs, rhs) {
      ...
      if (txnIdComparison != 0) {
        // we use the prevTxnId to sortqg
